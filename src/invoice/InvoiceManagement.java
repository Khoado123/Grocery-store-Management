...
public class InvoiceManagement {
    ...
    private static final String SERVER_NAME="LAPTOP-CEPEPGEL\\SQLEXPRESS01";
...
//    }
    ...
    public static SQLServerDataSource dataSource = configdaDataSource();
    public static void loadTableInvoice(JTable tb, String sql){
        
        try (var conn = dataSource.getConnection()) {
            DefaultTableModel model =  (DefaultTableModel) tb.getModel();
            model.setRowCount(0);
            java.sql.Statement st = conn.createStatement();
            SQLServerResultSet rs = (SQLServerResultSet) st.executeQuery(sql);
            while (rs.next()) {
                Object[] row = {
   ...
   public int writeInvoiceUsingPrepareStatement(Invoice invoice) {
    ...
    SQLServerDataSource ds = configdaDataSource();
    try (var conn = ds.getConnection()) {
        var sql = "INSERT INTO Invoice(emloyee_id,total_amount, payment_method, payment_status, created_at) VALUES (?,?, ?, ?, ?)";
        var ps = conn.prepareStatement(sql);
        ps.setInt (1,invoice.getEmployee_id());
        ps.setFloat(2, invoice.getTotal_amount());
        ps.setString(3, invoice.getPayment_method());
        boolean payment_status = invoice.isPayment_status();
        int paymentStatusInt = payment_status ? 1 : 0;
        ps.setInt(4, paymentStatusInt);
        ps.setTimestamp(5, invoice.getCreated_at());
